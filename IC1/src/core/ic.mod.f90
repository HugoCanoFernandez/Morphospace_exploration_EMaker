...
!******* #1 DEFINING SPATIAL DIMENSIONS *******
	...
	!epithelium's dimension parameters
	radi=re       !number of radial layers of nodes per cell
	radicel=rm    !number of radial layers of cells
	zepi=0d0     !z-position of basal layer
...
subroutine blastula
  ...
  call epi_growth_and_division(2,3)
  deallocate(gex)
  allocate(gex(nda,ng))
  deallocate(agex)
  allocate(agex(nda,ng))
    ...
    do i=1,ly
      do j=1,lx
        ...
        if(i==1.or.i==ly.or.j==1.or.j==lx)then
          ...
          !node(ii)%bor=1;node(ii-1)%bor=1;
          !if(ffu(17)==1)then
          !  nborders=nborders+1
          !  node(ii-1)%bor=nborders
          !  nborders=nborders+1
          !  node(ii)%bor=nborders
          !end if
        end if
      end do
    end do
    ...
    do k=1,layer
      ...
      do i=1,ly
        do j=1,lx
          ...
          if(i==1 .or. i==ly .or. j==1 .or. j==lx)then
            node(ii)%fix=2 ;node(ii)%rec=1d2 ; !node(ii)%bor=2 ;
            if(ffu(17)==1)then
              nborders=nborders+1
              node(ii)%bor=nborders
            end if
